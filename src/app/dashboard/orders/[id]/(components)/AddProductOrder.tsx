...
import { useSearchBusinessProducts, useUpdateOrder } from "@/actions";
...
const formSchema = z.object({
  title: z
    .string()
    .min(2, { message: "La busqueda debe tener al menos 2 caracteres" }),
});
interface Prop {
  order: Order;
}
  ...
  const add = ({
    ...
    mutate({
      ...
      orderDto: {
        products: [
          ...order.products,
          { price, quantity: 1, name, id: productOrderId },
        ],
      },
    });
  };
  ...
  return (
    <div className="flex flex-col gap-2 ">
      ...
      <Table>
        ...
        <TableBody>
          {data?.map(
            ({ id, amount, price, brand, category, tipo, description }) => (
              <TableRow key={id}>
