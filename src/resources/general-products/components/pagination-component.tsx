import { Pagination, PaginationContent, PaginationEllipsis, PaginationItem, PaginationLink, PaginationNext, PaginationPrevious } from '@/resources/shared/ui/pagination'
...
export const PaginationComponent = ({ meta, page, setPage }: Props) => {
				...
				{Array.from({ length: endPage - startPage + 1 }, (_, index: number) => {
					...
					return (
						<PaginationItem key={currentPage} className={`cursor-pointer rounded-lg ${page === currentPage ? 'bg-slate-500' : ''}`}>
							<PaginationLink onClick={() => setPage(currentPage)} size={undefined}>
								{currentPage}
							</PaginationLink>
						</PaginationItem>
					)
	...
	return (
		<Pagination>
			<PaginationContent>
				<PaginationItem>
					<PaginationPrevious className="cursor-pointer" onClick={handlePreviousPage} size={undefined} />
				</PaginationItem>
				<PaginationItems />
				...
				<PaginationItem>
					<PaginationNext className="cursor-pointer" onClick={handleNextPage} size={undefined} />
				</PaginationItem>
			</PaginationContent>
		</Pagination>
	)
