...
import { useCreateOrder } from '@/resources/orders/services/order-mutations'
	...
	const onSubmit = async (values: Order) => {
		...
		create({
			orderDto: {
				...
				businessName,
				products: cart.map(({ amount, brand, description, id, price, quantity, tipo }) => ({
					amount,
					brand,
					description,
					id,
				...
				<form onSubmit={form.handleSubmit(onSubmit)} onError={(error) => console.log(error)} className="space-y-8">
					...
					{deliveryMethod === 'envio' && (
						<>
							...
							<FormField control={form.control} name="references" render={({ field }) => <MyFormItem field={field} label="Referencias" />} />
						</>
					)}
